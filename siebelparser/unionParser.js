...
//module.exports = class JSSPropertySet{}; 
...
/* 20.3.0.0SIA[23073]PATCHSET20_3 */
...
(), SiebelApp.contentUpdater = {
        ...
        selectedEl.each(function () {
            ...
            if (src.indexOf("Javascript:") != -1 || src.indexOf("javascript:") != -1) {
                ...
                SiebelApp.AjaxRequestMgr.Get(src, function (e) {
                    ...
                    SiebelApp.S_App.GetEnablePerfHooks() && SiebelApp.S_App.GetTimer() && (SiebelApp.S_App.GetTimer().TimeGoToView("", "Have Layout"), SiebelApp.S_App.GetTimer().TimePopupApplet("", "Have Layout"));
                    ...
                    try {
                        ...
                        u = "";
                        o && (u = "title-preserved='" + HtmlEncode(o[1]) + "'", e = e.replace(o[0], "")),
                        ...
                        var a = $(that).find("table").not("[datatable]");
                        ...
                        SiebelJS.Log("[ContentLoader]  Error -> " + f.message)
                    }
                    finally {
                        $.ajaxSetup({
                            cache: s
                    ...
                    SiebelApp.S_App.GetEnablePerfHooks() && SiebelApp.S_App.GetTimer() && (SiebelApp.S_App.GetTimer().TimeGoToView("", "Processed Layout"), SiebelApp.S_App.GetTimer().TimePopupApplet("", "Processed Layout"));
                    var l = SiebelApp.contentUpdater.loadContent(bSetFree, bSync, deferred, requestedMethod);
                    if (SiebelApp.contentUpdater.callBackMap[src]) {
                        if (l == 0) {
                            var c = $("html").data("deferObj");
                            c ? c.done(function () {
                                deferred.resolve(),
    ...
    setAutoHeight: function (e) {
        ...
        for (var n = 0, r = e.length; n < r; n++)
            t = e.eq(n), t.is(":empty") && t.siblings().length === t.siblings(":empty").length && t.height("auto");
        t = null
    },
    viewLoaded: function (e, t) {
        var n = SiebelApp.contentUpdater.callBackMap[e];//.length > 1 ? [ ...SiebelApp.contentUpdater.callBackMap[e].filter(e => e.handler.toString() !== n[0].handler.toString())];
        if (n) {
			/*SiebelApp.contentUpdater.callBackMap[e] = SiebelApp.contentUpdater.callBackMap[e].length > 1 ? [SiebelApp.contentUpdater.callBackMap[e][0], ...SiebelApp.contentUpdater.callBackMap[e].filter(el => el.handler.toString() !== SiebelApp.contentUpdater.callBackMap[e][0].handler.toString())] : SiebelApp.contentUpdater.callBackMap[e];
            n = SiebelApp.contentUpdater.callBackMap[e];*/
			const isAttachRefresh = n.length > 1; //ADUSALIN
            for (var r = 0, i = isAttachRefresh ? 1 : n.length; r < i; r++)
                if (n[r] && n[r].handler)
                    if (!t)
    ...
    InitializeiFrame: function (e, t) {
        ...
        if (n) {
            ...
            s && (i.attr("data-src") === s[0] || this.InvalidateHistory
                (i.attr("data-src"), s[0])) ? (i[0].contentWindow.location.replace(s[1]), this.iframeArr[location.href] = null, i.attr("data-first", "true")) : i.attr("data-first", "true").attr("src", i.attr("data-src"))
        } else
    ...
    ProcessHistoryInfo: function (target, navurl, lastExecute) {
        ...
        frame[0] ? frame[0].contentWindow.location.replace(navurl) : (this.iframeArr[location.href] = [target, navurl], eval(lastExecute))
    },
